...
- **[양산 파이프라인 운영 정착]** Cython 변환 로직 재설계로 데이터 처리 속도를 약 **100배** 가속하고, palette PNG 양자화로 저장 용량을 약 **75%** 절감했습니다. 그 결과 일 약 2만 장의 wafer 이미지를 1시간 주기로 실시간 처리하는 운영 흐름으로 정착시켰습니다.
- **[현업 비즈니스 임팩트]** 기존 수작업 조회 (1회 약 48매 한계) 방식에서 벗어나, 대량의 lot 단위 누적 비교와 신규 불량 패턴 (Unknown) 조기 검출이 가능해졌습니다. 수작업 조회의 한 번 한 번 클릭으로 화면을 넘기는 한계를 벗어나, 분석 엔지니어가 lot 단위로 묶어 보고 신규 결함 후보까지 동시에 확인하는 흐름이 자리 잡았습니다. 2025년 양산 적용 이후 DRAM 전제품 라인에서 누적 운영 중이며, Unknown 후속 현업 확인 결과가 신규 결함 카탈로그 확장과 Known classifier 학습 sample 보강으로 이어지는 운영 cycle 이 정착된 상태입니다.
