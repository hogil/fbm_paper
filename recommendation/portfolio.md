...
- **기술 지표**
  ...
  - **[Unknown 추가 생성 데이터, 개발 중]** 현업 데이터와 최대한 유사하게 구성한 별도 평가셋(per class 500, normal 2000)에서, 구성요소를 단계적으로 더한 최종 recipe + 후처리(τ=0.5) 기준 신규 class **capture 0.9619** / **noise 0.00%** / **Completeness 0.9679** 와 군집 40→35 응집을 측정했습니다 (실전 운영 13/7 검증과 별도, 개발 단계 보조 지표).
  ...
  **Unknown contrastive 구성요소 성능표 [생성 데이터로 개발]**
  ...
  | # | Recipe (per class 500, normal 2000) | M1 capture | M2 noise % | M3 Completeness | M4 Homogeneity | ARI | AMI | n_cluster |
  |---|--------|--------------|--------------|-------------------|------------------|-----|-----|-----|
  | 1 | Global InfoNCE only (baseline) | 0.9337 | 15.78% | 0.9468 | 0.8348 | 0.5489 | 0.8855 | 40 |
  | 2 | + Local DenseCL (LW=0.5) | 0.9361 | 13.87% | 0.9502 | 0.8111 | 0.5314 | 0.8734 | 37 |
  | 3 | + MoCo Queue 4096 | 0.9356 | 9.45% | 0.9474 | 0.8368 | 0.5596 | 0.8870 | 41 |
  | 4 | + NV-Retriever NEG 0.72 | 0.9250 | 8.23% | 0.9485 | 0.8291 | 0.5683 | 0.8831 | 40 |
  | 5 | + NeCo 0.2 (Local 포함, 5-tool) | 0.9559 | 6.66% | 0.9660 | 0.8208 | 0.5648 | 0.8861 | 35 |
  | 6 | 최종 4-tool (Local 제외, 5-tool과 동일) | 0.9559 | 6.66% | 0.9660 | 0.8208 | 0.5648 | 0.8861 | 35 |
  | 7 | 최종 + 후처리 (noise 임계 τ=0.5) | 0.9619 | 0.00% | 0.9679 | 0.8184 | 0.5489 | 0.8856 | 35 |

  Local DenseCL 추가(5-tool)와 제외(4-tool)의 결과가 동일해(군집 35, capture 0.9559) 최종은 더 단순한 4-tool을 채택했습니다. ARI/AMI가 거의 변하지 않은 것은 강한 사전학습 backbone 위에서 contrastive head가 분리도 자체보다 noise 제거·capture·군집 응집 같은 운영 지표를 다듬기 때문입니다.
