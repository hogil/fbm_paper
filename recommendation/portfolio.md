...
학습 코드 (`_train_chip_variant.py::complement` 모드, `n_groups=2`, GRID=8, pair-fill white) 를 그대로 실제 chip 이미지에 적용한 결과입니다. 이 설정은 per-class 50 평가에서 가장 많이 검증된 group2 SOTA (iter26F: bit_F1 0.9953 / Total FAR 0.00%) 와 동일한 recipe 입니다 (1행 6열).
...
chip 을 8×8 격자 (총 64 cell) 로 나눈 뒤 무작위로 두 그룹 (각 32 cell) 으로 쪼갭니다. A label 쪽과 B label 쪽은 **서로 다른 분할** 을 한 번씩 따로 뽑아 사용합니다.
