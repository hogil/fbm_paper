...
└──────────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌──────────────────────────────────────────────────────────────────────────────┐
│  [VIEWER]  운영 뷰어 Web App (대량 wafer 조회 / lot 단위 누적 비교)          │
│  • 사내 인증 시스템 (SSO) 연동                                                │
│  • 일 약 **2만** wafer / **1시간 주기** batch 처리                            │
│  • 기존 수작업 조회 1회 ~48매 한계 → lot 단위 누적 비교 확장                  │
└──────────────────────────────────────────────────────────────────────────────┘
                                    │
                ┌───────────────────┴───────────────────┐
                ▼                                       ▼
...
두 모듈이 풀어야 하는 일 자체가 다릅니다. ROI-YOLO 는 wafer 전체 이미지만 보고 chip 위치를 모르는 상태에서 시작하기 때문에, 수많은 후보 box 를 두고 어디에 결함이 있는지, 어느 위치인지, 어느 class 인지 한꺼번에 추정해야 합니다. fail-map 처럼 작은 격자 안에 결함이 빽빽하게 몰리는 경우에는 box 회귀 오차와 class 분류 오차가 같이 누적되면서 정확도가 흔들립니다. 반면 chip-CNN object-id map 은 fail-map 파이프라인이 chip 좌표를 이미 확정해 둔 상태에서 들어가기 때문에, 모델 입장에서는 chip crop 한 장을 받아 분류만 하면 됩니다. detection 에서 박스를 찾고 좌표를 잡고 NMS 까지 거쳐야 하던 부분이 사라지니, V3 obj-only chipgrid 32×32×5 one-hot 기준으로 val_f1 **0.9946** / test_f1 0.9872 / 5-seed 평균 0.9838 ± 0.0092 까지 올라왔고 latency 도 짧아졌습니다. 이 차이가 Stage 2 모듈을 교체로 가져간 이유입니다. 같은 wafer 라도 raw map 으로는 비슷해 보이는 경우가, chip 단위 failure grid 로 옮겨 놓으면 center 영역 결함이 바로 보여서 헷갈리던 class 의 분리력을 한 번 더 끌어올릴 수 있습니다.
...
- **현업 임팩트**
  ...
  - **[추가 생성 데이터 / 공개 anchor 기반 후속 방법론 개발 중]**
    - 현재 P1 대표 성과 weighted F1 **0.95** 는 실제 현업 데이터에서 검증한 **ROI-YOLO 기반 2-stage 결과** 입니다. chip-CNN object-id map 은 추가 생성 chip 데이터로 개발 중인 **Stage 2 대체 모듈** 이고, 양산 적용 여부는 validation 검증 결과와 운영 절차에 따라 결정합니다.
    - chip-CNN object-id map 은 chip 분류만 수행하므로 class 가 추가될 때 annotation 과 재학습 비용이 크게 줄어듭니다 (구조 차이는 §(3) 후속 보정 단락에 자세히 풀어 두었습니다).
    - Unknown 검출은 추가 anchor 셋 기반 contrastive recipe (MoCo Queue / NV-Retriever / NeCo 등) 와 cross-anchor 일반화 개선을 별도 트랙으로 개발 중입니다.
    - 본 후속 개발 트랙의 정량 metric 은 위 현업 데이터 평가 성과와 혼용하지 않고 별도 관리합니다. chip-CNN object-id map 과 Unknown cross-anchor 일반화 평가는 대표 성과가 아닌 심화 질의 대비용 보조 지표로 구분하여 관리합니다.
...
DenseCL / MoCo Queue / NV-Retriever / τ=0.5 후처리는 운영 대표 성과가 아니라, embedding 을 더 잘 만들기 위한 후속 metric 관리 항목입니다. noise % 가 6.20% 에서 0.52% 까지 내려간 건 patch 수준 matching 과 hard-negative 제어를 차례로 쌓아 본 결과이고, 마지막에는 Local DenseCL 을 빼고 4-tool 만 남긴 구성이 안정성과 분리력 균형이 더 좋아 그 쪽으로 두고 보고 있습니다.
...
추가로 4-bag ensemble 과 KD 는 후속 운영성 검토 항목으로 분리했습니다. 4-bag ensemble 은 seed 와 augmentation 강도 차이에 따른 checkpoint 흔들림을 보는 보조 안정성 실험이고, KD single student 는 single-model 수준의 추론 비용으로 압축 가능성을 보는 후보입니다. 둘 다 bit_F1 이 대표 모델보다 낮은 구간이 있어 제출 성과에는 포함하지 않습니다.

**P2 대표 성과 / 후속 개발 / 한계 및 관리**

- **대표 성과**: FCM-PM val_margin 단일 모델 bit_F1 **0.9943** / Total FAR **0.00%** 입니다.
- **후속 개발**: KD single student 압축 후보, 4-bag ensemble 안정성 비교, 운영 threshold calibration 은 대표 성과와 분리해 관리합니다.
- **한계 및 관리**: P2 결과는 controlled benchmark 기준으로 해석합니다. 본 P2 의 failure class 는 실제 EDS Failbit Map 에서 관찰되는 failure 형태를 기준으로 구성했습니다. EDS Failbit Map 은 Test 결과가 0~7 Grade 로 표현된 map 이며, **Grade 0 은 양호, Grade 7 은 가장 강한 failure 상태**입니다. 본 P2 benchmark 는 이번 생성 데이터 기준으로 정의한 **single failure 4 class (bank_boundary / fork / scratch / scratch_rot)** 에서 가능한 **2-combo 6종 (bank_boundary+fork / bank_boundary+scratch / bank_boundary+scratch_rot / fork+scratch / fork+scratch_rot / scratch+scratch_rot)** 을 모두 포함하도록 구성했습니다. 따라서 해당 benchmark 범위 안에서는 2-combo 조합 누락 없이 평가했으며, 실제 운영에서 추가될 수 있는 신규 failure class 와 장기 운영 분포 변화는 후속 확장 및 검증 대상으로 분리합니다.
...
│ I13 max-prob<0.55 → Normal 강제 (운영 80% Normal 사전 정보)               │
│ pos=0.95 / neg=0.30 비대칭 target (39-cell sweep)                          │
│ 4-bag g=2/2/3/4 ensemble: teacher / 상한 확인 보조                         │
│ KD_v7 skip-on-cutmix: 1× cost 압축 후보 (6개 KD collapse 해소)             │
└──────────────────────────────────┬────────────────────────────────────────┘
                                   ▼
┌─ OUTPUT: run116J val_margin best (ep6) ─────────────────────────────────┐
...
- **후속 개발**: KD single student 압축 후보, 4-bag ensemble 안정성 비교, 운영 threshold calibration 으로 분리해 관리합니다.
...
|---|--------|--------|-----------|---------|------------|--------|-----------|
| 1 | BCE + Label Smoothing | 0.1093 | 99.47% | 1x | 1x | 1x | 2-combo 신호를 거의 학습하지 못해 기본 multi-label 구조만으로는 운영 후보가 되기 어렵습니다. |
| 2 | Sigmoid Focal Loss | 0.7980 | 45.72% | 1x | 1x | 1x | hard sample 대응은 됐지만 Normal / Invalid / OOD false-positive 가 남았습니다. |
| 3 | Asymmetric Loss (ASL) | 0.6435 | 100.0% | 1x | 1x | 1x | class imbalance 손실만으로는 negative rejection 이 무너졌습니다. |
| 4 | CutMix (random rectangle) | 0.9359 | 42.05% | 1x | 1x | 1x | bit_F1 은 회복됐지만 background 가 failure 로 학습되면서 FAR 42%, 운영 적용은 곤란합니다. |
| 5 | CutMix + Pair Mask | 0.9256 | 100.0% | 1x | 1x | 1x | mask 만 붙이면 임계 수렴으로 흔들렸고, failure signal coverage 보장이 같이 필요했습니다. |
| 6 | FCM-PM + val_f1 selection | 0.9422 | 0.00% | 1x | 1x | 1x | 같은 FCM-PM recipe 에서 val_f1 기준으로 고른 checkpoint 입니다. |
| 7 | FCM-PM + val_margin selection | 0.9943 | 0.00% | 1x | 1x | 1x | 제출 대표 모델입니다. bit_F1 과 false-positive 억제가 동시에 맞았습니다. |
| 8 | 4-bag Majority Voting Ensemble※ | 0.9167 | 4.05% | 4x | 1/4x | 4x | 단일 대표 모델 대비 bit_F1 / FAR 두 지표가 다 좋지 못한 보조 안정성 실험입니다. bag 별 CutMix grid 강도를 달리해 실수 패턴 겹침을 확인했고, 4x latency / 1/4x throughput 이라 운영 후보는 단일 모델로 고정했습니다. |
| 9 | Knowledge Distillation (single student) | 0.9265 | 0.00% | 1x | 1x | 1x | CutMix 활성 batch 에서 KD loss 를 제외해 반복적으로 발생한 학생 모델 collapse 를 막았습니다. 단일 대표 모델보다 bit_F1 이 낮아 후속 압축 후보로만 관리합니다. |
...
본 P2 benchmark 는 이번 생성 데이터 기준으로 정의한 single failure 4 class 에서 가능한 2-combo 6종을 모두 포함하도록 구성했습니다. 따라서 해당 benchmark 범위 안에서는 2-combo 조합 누락 없이 평가했으며, 실제 운영에서 추가될 수 있는 신규 failure class 와 장기 운영 분포 변화는 후속 확장 및 검증 대상으로 분리합니다. 본 결과는 현업 single failure chip 원천에서 출발해 도메인 확률분포 기반으로 구성한 controlled benchmark 기준이며, label 부족 조건에서 2-combo 학습 신호와 false alarm 억제 구조를 검증한 결과로 해석합니다.
...
│ Rendering: 224×224 PNG, train 7K + val 1.5K + test 1.5K = 10,000 scenarios│
└──────────────────────────────────┬────────────────────────────────────────┘
                                   │
                  ┌────────────────┴────────────────┐
                  ▼                                 ▼
┌─ Binary gate (정상 / 이상) ────┐  ┌─ Type classifier (이상 type) ─────────┐
│ F1 0.9967 (TN=746/FN=1/        │  │ accuracy 0.7303 (PoC)                  │
│            FP=4/TP=749)         │  │ context 100% / spike 66% / drift 28%   │
│ 5-seed (pc600_s1/pc700_s2):    │  │ 주 혼동: drift↔mean_shift 97건         │
│ F1 0.9987 (TN=748/FN=0/        │  │         / spike↔std 51건              │
│            FP=2/TP=750)         │  │ val-F1 median smoothing 보조           │
└────────────────┬───────────────┘  └────────────────┬──────────────────────┘
                 └───────────────┬───────────────────┘
                                 ▼
