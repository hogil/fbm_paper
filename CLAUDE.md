...
### Rule 15.1: 자료조사 / 자료생성 / 평가 에이전트 최신 dispatch 기준 (2026-06-19)

모든 자료조사, 자료생성, 평가 심사위원 에이전트는 아래 기준을 prompt에 포함하고 작업한다.

**공통 기준**:
- 최신 공식 발표 양식은 아래 6개 항목이다: `커리어 소개`, `과제 소개(개발배경/해결 문제/AI 적용 필요성/기술 stack)`, `개발내용(architecture/model/data pipeline)`, `성과 및 적용(KPI/공정 적용/현업 활용)`, `확장 가능성(타 공정/제품/운영유지보수)`, `본인기여`.
- 평가축은 `기술 전문성`, `기술 리더십`, `기술성과` 3개다. 모든 피드백은 이 3개 평가축 중 어디를 올리는지 명확해야 한다.
- P1 / P2 / P3를 하나의 pipeline으로 억지 연결하지 않는다. 각각 다른 대표 project로 보고, 시작은 3개 project summary다.
- P2/P3는 개발 중 project라도 발표 가능하다. 개발 중이라는 이유로 낮게 평가하지 말고, 기술 이해도 / 반도체 domain 적용력 / 검증 설계 / 설명력을 평가한다.
- `방어` 중심 피드백을 남발하지 않는다. 대신 임원 심사 목적에 맞게 `이해도`, `적용력`, `설명력`, `성과`, `확장성` 관점으로 평가한다.
- 성능표와 수치는 삭제 대상이 아니다. 원리 설명 뒤에 성능 개선 evidence로 남겨야 한다. 줄일 때는 표가 아니라 표 주변의 반복 설명과 중복 문장을 줄인다.
- 모든 핵심 기술 설명은 `왜 이걸 해야 했는가 -> 어떤 문제가 있었는가 -> 어떤 방법으로 해결했는가 -> 결과 수치가 어떻게 변했는가` 흐름을 갖춘다. 기법명만 나열하거나 "좋아졌다"로 끝내지 않는다.
- 마지막 장은 단순 closing 이 아니라 심사위원이 점수를 매기는 항목을 다시 떠올리게 하는 score-summary 장으로 설계한다. `기술 전문성 / 기술 리더십 / 기술성과` 3개 평가축에 맞춰 본인 기여와 결과를 요약한다.
...
- 각 기법별로 `문제 -> 적용 이유 -> 해결 구조 -> 개선 수치` evidence를 같이 수집한다. 예: `MoCo/NeCo -> embedding noise 감소`, `FCM-PM/val-margin -> bit_F1/FAR 개선`, `episode generator -> binary validation/robustness 확인`.

**자료생성 / 작성 에이전트 기준**:
- 슬라이드/문서 문장은 AI가 자동 작성한 것처럼 쓰지 않는다. 사람이 임원에게 설명하는 자연스러운 한국어와 필요한 영어 기술어를 섞어 쓴다.
- 각 project는 `원리 설명 -> 적용 방식 -> 성능/검증 evidence` 흐름으로 구성한다.
...
**평가 심사위원 에이전트 기준**:
- 점수는 `기술 전문성 / 기술 리더십 / 기술성과`로 나누어 준다.
- 기술 전문성: 문제를 AI 관점으로 정의했는지, 모델/실험으로 성능을 개선했는지, 데이터 파이프라인을 구성했는지, 재학습/모니터링 체계를 설명했는지 본다.
- 기술 리더십: 협업 범위, 기술 의사결정 전문성, 타 조직 확산, 코드/기술 공유, 현업 설명력과 coverage 확대를 본다.
- 기술성과: 양산 공정/제품 품질 적용, KPI 개선, 타 공정/제품 확장 가능성을 본다.
- P2/P3는 "실적이 없으니 약하다"가 아니라, 개발 중 인증 발표로서 기술 이해도와 검증 체계 설계가 충분한지 평가한다.
- 비기술 임원이 `이 사람이 어려운 AI 기술을 이해하고 우리에게 설명할 수 있다`고 느끼는지, 기술 임원이 `반도체 문제에 맞게 기법을 고르고 실험으로 개선했다`고 느끼는지를 동시에 본다.
- 각 기술 슬라이드를 볼 때 `왜/문제/해결/수치 변화` 중 빠진 요소를 지적한다. 단, 성능표 삭제를 권하지 말고 성능표 주변 문구를 정리하는 방향으로 제안한다.
- 마지막 장이 평가축 summary로 작동하는지 반드시 평가한다. 마지막 장에서 심사위원이 `기술 전문성`, `기술 리더십`, `기술성과` 점수를 쉽게 줄 수 있어야 한다.

### Rule 15.2: 합격률 직결 2대 레버 (2026-06-19, 사용자 직접 강조)
사용자 직접 메시지: "기술 설명들이 왜 이런 걸 하고 무슨 문제가 있었고 어떻게 해결해서 수치가 어떻게 변했는지가 부족하다. 심사위원도 사람이라 마지막에 점수 매기니, 마지막 장을 점수표 항목 summary로 만들어 점수받기용으로 만들면 합격률이 올라간다."
- 레버 1 (기술 설명 인과 흐름): 모든 핵심 기술 설명은 `왜 했는가 -> 어떤 문제가 있었는가 -> 어떤 방법으로 해결했는가 -> 결과 수치가 어떻게 변했는가` 4요소를 반드시 갖춘다. 기법명만 나열하거나 "좋아졌다"로 끝내면 감점 요인이다.
- 레버 2 (점수표 대응 마지막 장): 발표 마지막 장은 단순 closing 이 아니라 `기술 전문성 / 기술 리더십 / 기술성과` 3축 점수표에 그대로 대응하는 summary 장으로 만든다. 각 축에 본인 기여와 대표 evidence를 1~2개씩 배치해 심사위원이 점수를 바로 줄 수 있게 한다.
- 이 2대 레버는 자료생성 / 평가 에이전트 prompt (`.claude/team_agent_loop.ps1`) 와 발표 양식 rubric memory 에도 동일하게 반영한다.
...
## 발표 덱 (presentation) — 파생 작업 (2026-06)

추천서(.md)와 별개로, `recommendation/presentation/` 에서 같은 내용 기반 **AI Specialist 경연 임원 발표 덱(.pptx)** 을 제작 중.
**시작 시 `recommendation/presentation/HANDOFF.md` 를 먼저 읽을 것** (전체 상태·파일구조·진행방식).

### ★★★★★ 발표 심사 양식 절대규칙 (2026-06-19 최신 추가, 기존 덱 규칙보다 우선)

AI Specialist 면접 발표 덱을 평가/수정/스토리라인 작성할 때는 아래 공식 양식과 평가항목을 최상위 기준으로 삼는다. 과거 덱 가드레일과 충돌하면 이 섹션을 우선한다.
...
2. 참여/진행했던 과제 소개
   ...
   - 해결하고자 한 문제
   - AI 적용 필요성
   - 주요 AI framework / model / library 등 기술 stack 한 줄 요약
3. 개발 내용
   - architecture 설계
   - model 구현
   - data pipeline 등 기술 내용
4. 성과 및 적용
   - 정량적 성과
   - KPI 개선
   - 공정 적용 및 현업 활용 현황
5. 확장 가능성
   ...
   - 운영/유지보수 방안
6. 본인 기여
   - 과제 내 직접 기여 범위와 역할을 명확히 기재

**평가 항목 1 — 기술 전문성**:
- 문제를 AI 관점에서 명확히 정의하고 목표를 설정하였는가
- 모델 설계/구현/실험을 통해 성능을 개선하였는가
- 데이터 파이프라인을 체계적으로 구성하였는가
- 배포 후 재학습/모니터링 체계를 갖추었는가

**평가 항목 2 — 기술 리더십**:
- 협업 범위와 기술 의사결정 전문성이 있는가
- 개발 결과의 타 조직 확산 및 기술 coverage를 높였는가

**평가 항목 3 — 기술성과**:
- 양산 공정 또는 제품 품질에 AI를 직접 적용하였는가
- 핵심 KPI(수율, TAT 등) 개선에 기여하였는가
- 타 공정/제품으로의 확장 가능성이 있는가

**발표 덱 작성/평가 원칙**:
- P1 / P2 / P3는 하나의 pipeline으로 억지 연결하지 않는다. 각기 다른 대표 project 3개로 소개하고, 첫 장은 3개 과제 summary로 시작한다.
- 각 project는 `문제정의 -> AI 적용 필요성 -> 개발 내용 -> 성능/현업 적용 -> 확장 가능성 -> 본인 기여`가 보이도록 구성한다.
- 성능표와 수치는 삭제 대상이 아니다. 기술 원리 슬라이드 뒤에 성능/검증 표를 두어 "그래서 성능이 올라갔다"를 눈으로 보여준다.
- 기술 설명은 기법명 나열로 끝내지 않는다. 각 핵심 기법은 `왜 필요했는가 -> 기존 문제가 무엇이었나 -> 어떻게 해결했나 -> 수치가 어떻게 변했나`가 발표자가 말할 수 있게 구성한다.
- 비기술 임원에게는 어려운 기술을 쉽게 설명하는 능력을 보여주고, 기술 임원에게는 기법 이해도와 반도체 domain 적용력을 보여준다.
- "방어" 중심 표현보다 `기술 이해도`, `반도체 domain 적용력`, `설명력`, `성과와 확장성` 중심으로 평가/수정한다.
- P2/P3는 개발 중 project라도 발표 가능하다. 단, 운영/validation/PoC 상태 구분은 명확히 하되 불필요하게 낮춰 쓰지 않는다.
- NeCo, FCM-PM, val-margin, NB reject, object-id map, episode generator 같은 핵심 기법은 "왜 반도체 문제에 맞는지"까지 설명한다.
- 마지막 장은 `점수받기용 summary`로 설계한다. 심사위원 평가표의 `기술 전문성 / 기술 리더십 / 기술성과` 3개 축을 직접 겨냥해, 각 축별 대표 evidence를 정리한다.
- 마지막 장의 목적은 감성적인 closing이 아니라, 심사위원이 점수 입력 직전에 `문제정의·모델/실험·데이터 파이프라인·현업 적용·KPI·확장성·본인기여`를 한 번에 떠올리게 하는 것이다.

### ★ 덱 절대 가드레일 (위반 금지)
1. **슬라이드 17장 유지** — 추가·삭제 금지(품질만 개선).
2. **이미지는 `recommendation/figures` 원본만** — 가공·생성·합성·crop·대비보정 금지(_view/_focus/_hc/_v2 금지).
3. **웨이퍼 등 실데이터에 추측성 주석(원/화살표/색채움) 금지** — 실제 결함 좌표를 모르면 틀린 위치를 강조하게 됨.
4. **방어적 각주·면책 문구 금지**('척도 달라 동급 비교 아님', 'F1=…정도' 정의 등). PoC/생성셋 표기는 결과 괄호로 충분.
5. **'한눈에 카드'·'데이터 파이프라인' 슬라이드는 확정본** — 구조·기술표기 보존.
6. 자기과시 슬로건 금지, 일·결과 중심. 발표자 소개·성과 stats 페이지 금지.
7. 화이트 배경·여백 최소·네이비(#0F1E3D)/틸(#12B5B0), 모든 슬라이드 시각요소(글만 슬라이드 금지).

### 덱 정확성 / 엔진
- 양산 운영 중 = 데이터 파이프라인 + 운영 뷰어뿐. 모델(Known/Unknown) 미배포(GPU 2026.9). **F1 0.95=검증 성능**. P2/P3=생성셋·PoC.
- 기술스택: 생성=**Cython**+palette PNG(Pillow,75%↓)+ProcessPool / 운영뷰어=**Numba**(composite map)+**pyvips**(대량 이미지 로드)+pyramid·cache.
- 엔진=`build.py`(python-pptx), 렌더=`render_pptx.py`(COM→PDF→PNG+컨택트시트). 빌드 `python build.py spec.json 발표_AI인증_최호길.pptx`.
- 품질 향상은 **리파인 전용 워크플로우**(research/writer 없이 현재 spec만 평가→마스터수정).
