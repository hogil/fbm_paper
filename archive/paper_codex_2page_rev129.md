...
전체 운영 기준 하루 약 2만 장 이상의 Wafer가 1시간 주기로 처리되므로, backbone 선택에서는 정확도와 추론 처리량을 함께 고려하였다. MaxViT[4]와 ConvNeXtV2 (Ref)는 동일한 test Weighted F1 0.87을 보였으나, ConvNeXtV2가 1시간 주기 처리 요건을 더 안정적으로 만족할 수 있어 최종 backbone으로 선정하였다. 선정된 ConvNeXtV2 (Ref)의 test Weighted F1 0.87은 Ref + Optuna에서 0.92, Ref + Optuna + ROI에서 0.95로 단계적으로 향상되었다.
