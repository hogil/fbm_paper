...
하루 약 2만 장 이상의 Wafer Failbit Map이 발생하므로 backbone 선택에서는 정확도와 추론 처리량을 함께 고려하였다. MaxViT[4]와 ConvNeXtV2 (Ref)는 동일한 test Weighted F1 0.87을 보였으나, ConvNeXtV2는 파라미터 수와 FLOPs가 각각 약 26%, 39% 낮아 최종 backbone으로 선정하였다. 이후 Optuna 기반 hyperparameter 최적화로 test Weighted F1을 0.92까지 높였고, ROI-YOLO 보정으로 최종 0.95를 달성하였다.
